...
def create_thumbnail(video_file, output_file, time_offset):
    try:
        ffmpeg.input(video_file, ss=time_offset).output(output_file, vframes=1, y='-y', hide_banner=None, loglevel="quiet").run()
...
def thumbnail_upload(filepath):
    try:
        s3 = boto3.client('s3', aws_access_key_id=CLOUDFLARE_ACCESS_KEY, aws_secret_access_key=CLOUDFLARE_SECRET_KEY, endpoint_url=CONTENT_ENDPOINT, config=boto3.session.Config(signature_version='s3v4'))
        s3.upload_file(filepath, CONTENT_BUCKET, filepath)
    except FileNotFoundError:
        raise FileNotFoundError("Image file not found.")
...
def video_fetch_data(data):
    filename = urljoin(data["format"]["filename"], urlparse(data["format"]["filename"]).path)
...
def find_loudest_section(audio, sr, duration):
    ...
    max_energy = np.argmax([np.sum(rms[i:i+frames_per_chunk]) for i in range(0, num_frames, frames_per_chunk)])
...
def audio_image(url, file_ext, savepath):
    ...
    plt.savefig(savepath, bbox_inches='tight', pad_inches=0, facecolor=fig.get_facecolor(), edgecolor='none')
    plt.close()
